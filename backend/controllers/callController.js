...
const handleIncomingCall = asyncHandler(async (req, res) => {
  ...
  // A call ID allows a client to continue collecting details for the same call.
  if (req.body.callId) {
    call = await Call.findOne({ callId: req.body.callId });

    if (call) {
      if (call.phone !== normalizedPhone) {
        throw new AppError("callId does not belong to this phone number", 409);
      }

      if (["completed", "failed", "cancelled"].includes(call.status)) {
        throw new AppError("Cannot continue a closed call", 409);
      }

      conversation = await Conversation.findOne({ callId: call.callId });
      if (!conversation) {
  ...
  if (!call) {
    call = await Call.create({
      callId: req.body.callId || makeCallId(),
      phone: normalizedPhone,
      customerId: customer?._id,
    });
...
const updateCallStatus = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    throw new AppError("Cannot change the status of a closed call", 409);
  }
...
const endCall = asyncHandler(async (req, res) => {
  ...
  if (["completed", "failed", "cancelled"].includes(call.status)) {
    throw new AppError("Call is already closed", 409);
  }
