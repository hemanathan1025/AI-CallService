...
function validateCustomer({ addressRequired = false } = {}) { return (req, res, next) => {
  ...
  if (!text(name) || normalizePhone(phone).length !== 10) return next(new AppError("A valid name and phone number are required", 400));
...
function validatePhoneParam(req, res, next) { if (normalizePhone(req.params.phone).length !== 10) return next(new AppError("Phone number is invalid", 400)); next(); }
function validateOrder(req, res, next) {
  const { phone, items, amount, status } = req.body;
  if (phone !== undefined && normalizePhone(phone).length !== 10) return next(new AppError("Phone number is invalid", 400));
...
function requireOrderFields(req, res, next) { validateOrder(req, res, (err) => { if (err) return next(err); if (normalizePhone(req.body.phone).length !== 10 || !text(req.body.items) || req.body.amount === undefined) return next(new AppError("Phone, items and amount are required", 400)); next(); }); }
function validateCall(req, res, next) { if (normalizePhone(req.body.phone).length !== 10) return next(new AppError("A valid phone number is required", 400)); if (req.body.callId !== undefined && !text(req.body.callId)) return next(new AppError("callId must be a non-empty string", 400)); next(); }
